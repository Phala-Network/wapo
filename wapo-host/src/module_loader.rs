...
struct ModuleLoaderState {
    cache: LruCache<String, WasmModule>,
    compiling: BTreeMap<String, Sender<Result<WasmModule, ArcError>>>,
...
impl ModuleLoader {
    ...
    pub async fn load_module(&self, code_hash: &str) -> Result<WasmModule> {
        ...
        let mut module_rx = {
            ...
            match state.compiling.get(code_hash) {
                ...
                None => {
                    ...
                    state.queue.insert(code_hash.to_string());
                    let (tx, rx) = channel(1);
                    state.compiling.insert(code_hash.to_string(), tx);
    ...
    fn compile(&self, code_hash: &str) -> Result<WasmModule> {
        ...
        loop {
            let code_hash = {
                let mut state = self
                    ...
                    .lock()
                    .expect("BUG: ModuleLoaderState lock poisoned");
                let Some(hash) = state.queue.pop_first() else {
                    state.compiling_tasks -= 1;
            ...
            let _span = tracing::info_span!("compilation", code = code_hash).entered();
            let module = self.compile(&code_hash);
            let mut state = self
                .state
                .lock()
                .expect("BUG: ModuleLoaderState lock poisoned");
            let Some(tx) = state.compiling.remove(&code_hash) else {
                info!(target: "wapo", "BUG: missing compiling task for code {}", code_hash);
                continue;
            };
            if let Ok(ref module) = &module {
                state.cache.put(code_hash.to_string(), module.clone());
            }
