use crate::{IntPtr, IntRet, OcallError, Result};
use log::Level;
...
mod decode {
    use super::*;
    use wiggle::{GuestMemory, GuestPtr, GuestSlice, GuestSliceMut};

    pub(crate) trait ArgDecode<'d, 'r> {
        type Encoded;
        type Repr;
        fn decode_arg<R>(
            stack: StackedArgs<(Self::Encoded, R)>,
            vm: &'d dyn GuestMemory,
        ) -> Result<(Arg<'d, 'r, Self>, StackedArgs<R>)>
        where
            Self: Sized;
        fn decode_repr(repr: &'r mut Self::Repr) -> Result<Self>
        where
            Self: Sized;
    }

    pub(crate) struct Arg<'d, 'r, T: ArgDecode<'d, 'r>>(<T as ArgDecode<'d, 'r>>::Repr);
    impl<'d, 'r, T: ArgDecode<'d, 'r>> Arg<'d, 'r, T> {
        pub(crate) fn new(repr: T::Repr) -> Self {
            Arg(repr)
        }

        pub(crate) fn extract(&'r mut self) -> Result<T>
        where
            'd: 'r,
        {
            T::decode_repr(&mut self.0)
        }
    }

    impl<A, B> StackedArgs<(A, B)> {
        pub(crate) fn pop_arg<'d: 'r, 'r, T: ArgDecode<'d, 'r, Encoded = A>>(
            self,
            mem: &'d dyn GuestMemory,
        ) -> Result<(Arg<'d, 'r, T>, StackedArgs<B>)> {
            T::decode_arg(self, mem)
        }
    }

    impl<'d: 'r, 'r: 'a, 'a> ArgDecode<'d, 'r> for &'a [u8] {
        type Encoded = (IntPtr, IntPtr);

        type Repr = GuestSlice<'d, u8>;
            ...
            Self: Sized,
        {
            ...
            let repr: GuestSlice<'_, u8> = GuestPtr::new(mem, (ptr as u32, len as u32))
                .as_slice()
    ...
    impl<'d: 'r, 'r: 'a, 'a> ArgDecode<'d, 'r> for &'a str {
        type Encoded = (IntPtr, IntPtr);
        type Repr = GuestSlice<'d, u8>;
        ...
        fn decode_arg<A>(
            ...
            vm: &'d dyn GuestMemory,
        ) -> Result<(Arg<'d, 'r, Self>, StackedArgs<A>)>
        where
            Self: Sized,
        {
            let (inner, stack) = <&[u8]>::decode_arg(stack, vm)?;
            Ok((Arg(inner.0), stack))
        }

        fn decode_repr(repr: &'r mut Self::Repr) -> Result<Self> {
            core::str::from_utf8(&*repr).or(Err(OcallError::InvalidEncoding))
        }
    }

    impl<'d: 'r, 'r: 'a, 'a> ArgDecode<'d, 'r> for &'a mut [u8] {
        type Encoded = (IntPtr, IntPtr);
        type Repr = GuestSliceMut<'d, u8>;
        ...
        {
            let ((len, ptr), stack) = stack.pop();
            let repr: GuestSliceMut<'_, u8> = GuestPtr::new(mem, (ptr as u32, len as u32))
                .as_slice_mut()
                .map_err(|_| OcallError::InvalidParameter)?
                .ok_or(OcallError::InvalidParameter)?;
            Ok((Arg(repr), stack))
...
macro_rules! impl_codec64 {
    ($typ: ty) => {
        ...
        #[cfg(feature = "host")]
        impl<'d: 'r, 'r> decode::ArgDecode<'d, 'r> for $typ {
            type Encoded = (IntPtr, IntPtr);
            type Repr = Self;

            fn decode_arg<R>(
                stack: StackedArgs<(Self::Encoded, R)>,
                _vm: &'d dyn wiggle::GuestMemory,
            ) -> Result<(decode::Arg<'d, 'r, Self>, StackedArgs<R>)>
            where
                Self: Sized,
            {
...
#[cfg(feature = "host")]
impl<'d: 'r, 'r, I: I32Convertible> decode::ArgDecode<'d, 'r> for I {
    type Encoded = IntPtr;
    type Repr = i32;
        ...
        Self: Sized,
    {
        ...
        Ok((decode::Arg::new(v as _), stack))
    }
